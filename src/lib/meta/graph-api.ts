...
// chegou a virar conexão: a Meta compartilha a conta do cliente com o negócio
// nesse momento, mesmo quando o popup não consegue nos avisar.
//
// Só a borda de clientes, nunca `owned_whatsapp_business_accounts`: as contas
// da própria Autoprocs não chegaram por esse caminho e não têm o que fazer
// numa lista de recuperação dentro da tela de um cliente.
// ----------------------------------------------------------------------------
...
export async function listClientWabas(
  ...
  try {
    const data = await request<BusinessWabaList>(
      `/${businessId}/client_whatsapp_business_accounts`,
      { method: "GET", token, query: { fields: "id,name", limit: "100" } },
    );
    return (data.data ?? []).map((waba) => ({ id: waba.id, name: waba.name ?? null }));
  } catch {
    // Negócio que não expõe a borda: nada a recuperar por ele.
    return [];
  }
}
